defmodule PGS.Player do
    ...
    def onCreate do
        Sys.Log.debug("#{Sys.Game.name}-#{Sys.Game.stage} New player (#{inspect Sys.Player.pKey}) has joined the game!")
    ...
    def onLogin do
        Sys.Log.debug("#{Sys.Player.pKey} logged in to #{Sys.Game.name}-#{Sys.Game.stage}")
        :ok
    end

    def onLogout do
        Sys.Log.debug("#{Sys.Player.pKey} logged out of #{Sys.Game.name}-#{Sys.Game.stage}")
        :ok
    end

    def onStart do
        Sys.Log.debug("#{Sys.Player.pKey} Process started in to #{Sys.Game.name}-#{Sys.Game.stage}")
        :ok
    end

    def onStop(reason) do
        Sys.Log.debug("#{Sys.Player.pKey} Process Stopped Simple-Base because of #{inspect reason}")
        :ok
    end
    ...
    def onMessage("sendMail", _data) do
        Sys.Player.mailSend(Sys.Player.pKey, "The subject!", "Hi #{Sys.Player.name}! You sent yourself a mail.")
    end

    def onMessage("sendReward", _data) do
        Sys.Player.mailSend(Sys.Player.pKey,  "Rewards",
                                              "You got some ore!",
